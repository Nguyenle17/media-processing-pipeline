...
export default function Home() {
    ...
    const handleSubmitVideo = async () => {
        try {
            ...
            const jobResponse = await Api.post('/job/create', { title: videoFile.name, countChunks: 0, totalChunks: 1, duration: range[0] - range[1] });
            ...
            await uploadWithProgress(formData, (pct) => { setPercentage(pct); setStatus(`Uploading... ${pct}%`); });

            setPhase('transcribing'); setPercentage(0); setStatus('Transcribing...');
        ...
        for (let i = 0; ; i++) {
            ...
            if (job.status === 'processing') {
                ...
                if (job.processedChunks && job.totalChunks) {
                    setPercentage(Math.round((job.processedChunks / job.totalChunks) * 100));
                    setStatus(`Transcribing... ${job.processedChunks}/${job.totalChunks} chunks`);
                }
            }

            if (job.status === 'waitting') {
            ...
            if (job.status === 'failed') throw new Error('Transcribe failed');
    ...
    const handleFixGrammar = async () => {
        try {
            ...
            setLoading(true); setStatus('Fixing grammar...'); setPhase('transcribing');
            const response = await Api.post('/video/grammar', { text: text });
            setText(response.correctedText); setStatus('Grammar fixed!'); setPhase('done');
        } catch (error) {
            setStatus('Error: ' + error.message); setPhase('done');
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <section style={{ fontFamily: "'Syne', sans-serif", background: '#0a0a0f', minHeight: '100vh', color: '#fff', margin: 0, padding: 0 }}>
            <style>{`
                .hero-section {
                    min-height: 100vh;
                    display: flex;
                ...
                .hero-title .accent { color: transparent; background: linear-gradient(135deg, #6366f1, #a78bfa, #ec4899); -webkit-background-clip: text; background-clip: text; }
                .hero-sub { font-size: 18px; color: #71717a; line-height: 1.6; max-width: 480px; margin: 0 auto 40px; font-weight: 400; }
                .cta-btn {
                ...
                .stat-lbl { font-size: 12px; color: #52525b; text-transform: uppercase; letter-spacing: 0.1em; font-family: 'JetBrains Mono', monospace; }
                ...
                .result-empty-icon { font-size: 32px; opacity: 0.2; }
                ...
                .loading-panel {
                    display: flex; flex-direction: column; align-items: center;
                    justify-content: center; min-height: 200px; gap: 20px;
                }
                ...
                <div className="studio-grid">
                    ...
                    <div className="panel fade-in">
                        <div className="panel-header">
                            <div className="panel-dot" style={mode === 0 ? { background: '#f59e0b' } : { background: '#26f50b' }} />
                            <span className="panel-title">input.mp4/.mp3/.wav/.m4a</span>
                        </div>
                        <div className="panel-body">
                            <video controls src={videoURL || ''} onLoadedMetadata={handleVideoLoaded} style={{ background: '#0d0d12' }}>
                                {!videoURL && <p style={{ color: '#3f3f46', fontSize: 13, textAlign: 'center', padding: 40 }}>No video selected</p>}
                            </video>
                                ...
                                videoURL ? (
                                    <>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', margin: 16 }}>
                                            <span style={{ fontSize: 13, color: '#8284f9', fontFamily: "'JetBrains Mono', monospace" }}>
                                                {formatTime(range[0])}
                                            </span>
                                    ...
                                    <div className="flex gap-4 mt-6">
                                        ...
                                        <button
                                            ...
                                            style={{
                                                ...
                                                background: 'rgba(89, 16, 185, 0.12)',
                                                border: '1px solid rgba(117, 16, 185, 0.3)',
                                                color: '#a86ee7', fontSize: 12, cursor: 'pointer',
                                        ...
                                        <button className="btn" style={{
                                            display: 'flex', alignItems: 'center', gap: 6,
                                            padding: '7px 16px', borderRadius: 8,
                                            background: 'rgba(185, 61, 16, 0.12)',
                                            border: '1px solid rgba(185, 16, 16, 0.3)',
                                            color: '#e76e6e', fontSize: 12, cursor: 'pointer',
                                            fontFamily: "'JetBrains Mono', monospace",
                                            transition: 'all 0.15s', whiteSpace: 'nowrap',
                                            width: 'fit-content',
                                        }}
                                            ...
                                            onClick={handleFixGrammar}>🔧 Grammar</button>

                                    </div>
                                    <div className="mt-5" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        {/* File name */}
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                            <label style={{ fontSize: 11, color: '#71717a', fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                                                File name
                                            </label>
                                            <input
                                                type="text"
                                                defaultValue={`transcript_${Date.now()}`}
                                                style={{
                                        ...
                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                            <label style={{ fontSize: 11, color: '#71717a', fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>
                                                Export as
                                            </label>
                                            <select style={{
                                                background: 'rgba(255,255,255,0.05)',
